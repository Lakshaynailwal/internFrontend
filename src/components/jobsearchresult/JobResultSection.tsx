...
export const JobResultSection = ()=>{
    ...
    useEffect(()=>{
      fetch(`${URL}/jobs?page=${page}&filter=${input}`).then(async (res)=>{
    ...
    },[page, input ,setJobList , setTotal])
    ...
    return(
        ...
        <div className="flex gap-2">
          {/* prev control */}
            ...
            <p className=" text-center bg-slate-100 w-16 border-2 rounded-md"> {page}/{Math.ceil(total/6) >0 ? Math.ceil(total/6) : 1} </p>
            {/* next control */}
            <button onClick={()=>{
